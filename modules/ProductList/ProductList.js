...
export class ProductList {
  ...
  getHTMLTemplateListItem({id, price, name: title, images: [img]}) {
    return `
      <article class="goods__card card">
        <a  class="card__link card__link_img" href=/product/${id}>
          <img class="card__img" src=${API_URL}${img} alt=${title}>
        </a>
  ...
  renderPagination(paginationData) {
    ...
    pagination.insertAdjacentHTML('afterbegin', this.getPagination(paginationData))
    
    this.containerElement.append(pagination);
  }

  getPagination({currentPage, totalPages}) {    
    return `
      <div class="pagination__bar" style="--width: calc(${currentPage} / ${totalPages} * 100%)"></div>
